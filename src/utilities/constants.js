module.exports = constants = {
    STATUS_SUCCESS: "success",
    STATUS_FAILURE: "failure",
    SOMETHING_WRONG_MSG: "Something went wrong",
    
    LOCALE: "en",
    USER_REGISTER_SUCCESS_MSG: "User successfully registered",
    LOGIN_SUCCESS: "Logged In successful",
    USER_UPDATED_SUCCESS_MSG: "user successfully updated!",
    BAD_REQ_MSG: "Bad Request",
    SERVER_ERR: "Internal server error",
    USER_NOT_FOUND_WITH_EMAIL_MSG: "User not found with this email",
    USER_EXIST_WITH_EMAIL: "User already exist with this email!",
    USER_EXIST_WITH_USERNAME: "User already exist with this username!",
   
    USER_LOGOUT_MSG: "User successfully Logout",
    USER_ID_NOT_FOUND: "User id is not there!",
   
   
    INVALID_OBJECTID : "Invalid objectId id",
    FORBIDDEN_MSG: "Forbidden! You're not authorized to perform this action",
    USER_ID_NOT_MATCH:"Request param User id does not matched with loggedIn User id",
    FILE_EXTENSIONS: ["png", "jpeg", "jpg"],
    
    
    
    MESSAGE_PRODUCT: {
      ADD: "bucket added successfully",
      FETCH:"bucket fetched successfully",
      DELETE : "bucket successfully deleted!",
      UPDATE : "bucket data updated successfully",
      NOT_FOUND: "bucket not found!",
      BUCKET_EXIST:"bucket name is already exist"
    },
    MESSAGE_FILE: {
      ADD: "File added successfully",
      FETCH:"File fetched successfully",
      DELETE : "File successfully deleted!",
      UPDATE : "File data updated successfully",
      NOT_FOUND: "File not found!",
    },
    
    //HTTP Status codes
    HTTP_200_CODE: 200,
    HTTP_201_CODE: 201,
    HTTP_204_CODE: 204,
    HTTP_401_CODE: 401,
    HTTP_403_CODE: 403,
    HTTP_404_CODE: 404,
    HTTP_409_CODE: 409,
    HTTP_422_CODE: 422,
    HTTP_400_CODE: 400,
    HTTP_206_CODE: 206,
    HTTP_500_CODE: 500,
  
  };
  